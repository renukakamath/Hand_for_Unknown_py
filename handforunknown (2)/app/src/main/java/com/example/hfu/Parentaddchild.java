...
public class Parentaddchild extends Activity implements JsonResponse, AdapterView.OnItemClickListener {

	EditText e1,e2,e3;
	Button b1;
	RadioButton r1,r2;
    String name,dob,gender,desc;
    
    ListView lv;
	String[] id,names,dod,genders,details,des;
	String logid;
...
//				startActivity(new Intent(getApplicationContext(),Login.class));
				...
				JsonReq jr= new JsonReq();
				jr.json_response=(JsonResponse) Parentaddchild.this;
				String q="childregister/?name="+name+"&gender="+gender+"&dob="+dob+"&logid="+logid+"&desc="+desc;
				q.replace("", "%20");
				jr.execute(q);
...
public void response(JSONObject jo) {
	...
	public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
		cid=id[i];
		final CharSequence[] items = {"Update Child Details","Check Symptoms", "Cancel"};

		AlertDialog.Builder builder = new AlertDialog.Builder(Parentaddchild.this);
		...
		builder.setItems(items, new DialogInterface.OnClickListener()
		{
			...
			public void onClick(DialogInterface dialog, int item) {
				...
				if (items[item].equals("Update Child Details")) {
					startActivity(new Intent(getApplicationContext(), Updatechilddetails.class));
				}else if (items[item].equals("Check Symptoms"))
				{
					startActivity(new Intent(getApplicationContext(), Checksymptoms.class));
